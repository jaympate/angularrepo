    ...
    <div class="projects-block">
      <div class="card projects-content-left" [ngClass]="left ? 'projects-content-left' : 'projects-content-right'">
        <div class="card-header">
          <span><i class="fa" [ngClass]="project.icon"></i> {{project.client}}</span>
        </div>
        <div class="card-body">
          ...
          <div *ngIf="!!project.technologies" class="text-center">
            <span *ngFor="let technology of project.technologies" class="badge badge-pill badge-dark ml-1">{{ technology }}</span>
          </div>
        </div>
        <span [ngClass]="left ? 'date-right' : 'date-left'">{{ project.timeSpan }}</span>
