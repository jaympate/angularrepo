export class Languages {
  ...
  private constructor(currentLanguage: string,
  ...
  private static assertNotEmpty(supportedLanguages: string[]) {
    if (supportedLanguages.length === 0) {
      throw new Error('Invalid argument exception: Supported languages cannot be empty.');
    }
  }

  private static assertIncludes(supportedLanguages: string[], currentLanguage: string) {
    if (!supportedLanguages.includes(currentLanguage)) {
      throw new Error('Invalid argument exception: Supported languages should include the current language.');
    }
  }
}
