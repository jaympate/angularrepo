...
export class TranslationService {
  ...
  getTranslation(language: string): Observable<TranslationKeyValues> {
    return combineLatest([
      this.getStaticTranslation(language),
      this.getBackendTranslation(language)
    ]).pipe(
      map(([staticTranslations, backendTranslations]) => ({
        ...staticTranslations,
        ...backendTranslations
      }))
    );
  ...
  private getStaticTranslation(
    ...
    if (!this.staticTranslationCache.has(language)) {
      this.staticTranslationCache.set(
        language,
        this.getLanguageTranslationsRequest(language)
      );
    }
  ...
  private getBackendTranslation(
    language: string
  ): Promise<TranslationKeyValues> {
    if (!this.backendTranslationCache) {
      this.backendTranslationCache = this.backendTranslationService
        .getBackendTranslations$()
        .toPromise();
    }
    return this.backendTranslationCache.then(
      (backendTranslations) => backendTranslations.translations[language]
    );
  }
}
